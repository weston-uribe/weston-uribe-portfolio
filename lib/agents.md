...
- **Natural Language Robotics:** `nl-robotics.ts` — meta, hero, at-a-glance, simulation demos, three phases, technical-stack cards, factory-flow steps, run-comparison cards, factory callout, related Harness callout, and `NL_ROBOTICS_FUTURE_MEDIA_PATHS` (reserved, non-rendered). Use explicit `NlRoboticsSectionKey` / `NlRoboticsPhaseKey` mapped types on `NL_ROBOTICS_SECTIONS` and `NL_ROBOTICS_PHASES`; do not embed prose in the route page. Demo videos stage in `.local-media/nl-robotics/` (excluded from Git) before a follow-up import task replaces placeholders.

...
- **Harness:** `harness.ts` — meta, hero, at-a-glance, six phases, workflow comparison, operator steps, agent roles, evaluation dimensions, capabilities, roadmap, and install callout. Hero image at `public/images/harness-work-card.png`. Use literal-key typing on `HARNESS_SECTIONS` and `HARNESS_PHASES`; do not embed prose in the route page.
